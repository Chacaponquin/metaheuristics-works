...
from operators import random_list, min_value_list, next_list, random_crossing
from new_operators import add_or_minus_one_param, add_one_minus_one
...
def random_solution(step: float) -> list[float]:
...
def random_change(solution: list[float], step: float):
    ...
    if option == 1:
        ...
        return add_one_minus_one(ks=solution, step=step)
